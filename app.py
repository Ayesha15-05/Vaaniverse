...
class SignPlayer:
    ...
    def _next_gif_frame(self):
        if not self.gif_running:
            return
        try:
            if self.gif_idx >= len(self.gif_frames):
                self.gif_idx = 0
            if self.gif_frames:
...
class ModernButton(tk.Button):
    def __init__(self, master=None, **kwargs):
        ...
        self.config(bg=self.bg_color, fg=self.fg_color, font=self.font, bd=0, 
                   relief="flat", cursor="hand2", padx=self.padding[0], pady=self.padding[1])
...
class VaaniVerse(tk.Tk):
    def __init__(self):
        ...
        self.listening_active = False  # For waveform animation
        self.prev_lh = None
        self.prev_rh = None
    ...
    def _draw_ai_toggle(self):
        ...
        # background
        def create_rounded_rect(x1, y1, x2, y2, r, **kw):
        ...
        create_rounded_rect(2, 2, 44, 22, 10, fill=bg, outline="")
    ...
    def check_queue(self):
        try:
            while True:
                ...
                if msg[0] == 'status':
                    self.update_speak_status(msg[1], msg[2])
                elif msg[0] == 'text':
                    self.process_speech_text(msg[1])
        except queue.Empty:
            pass
        finally:
    ...
    def init_wave_animation(self):
        """Initialize the waveform bars on the canvas"""
        self.wave_bars = []
        bar_width = 8
        spacing = 4
    ...
    def animate_siri_wave(self):
        """Animate the waveform - pulsing animation without mic access"""
        pulse_height = 0
        pulse_direction = 1
        
        while self.listening_active:
            try:
                # Simple pulsing animation (no mic access)
                pulse_height += pulse_direction * 0.5
                ...
                # Update bars with pulsing height
    ...
    def build_ui(self):
        ...
        tk.Label(h, text="VaaniVerse", font=("Segoe UI", 24, "bold"), 
                fg="#F8FAFC", bg=self.COLORS['primary']).pack(side="left", padx=20)
        ...
        # Waveform Canvas for Siri-style animation
        ...
        lb = tk.Frame(left.inner, bg=self.COLORS['background'])
        lb.pack(side="bottom", fill="x", pady=10)
        
        ModernButton(lb, text="▶ Start", command=self.start_cam, 
                    bg_color=self.COLORS['success']).pack(side="left", fill="x", expand=True, padx=2)
        ModernButton(lb, text="⏹ Stop", command=self.stop_cam, 
                    bg_color=self.COLORS['danger']).pack(side="left", fill="x", expand=True, padx=2)
        ...
        rb = tk.Frame(right.inner, bg=self.COLORS['background'])
        rb.pack(side="bottom", fill="x", pady=10)
        
        ModernButton(rb, text="🔊 Speak (English)", command=lambda: self.speak_txt("eng"), 
                    bg_color=self.COLORS['accent']).pack(fill="x", pady=2)
        ModernButton(rb, text="🗣️ Speak (Translated)", command=lambda: self.speak_txt("phonetic"), 
                    bg_color=self.COLORS['secondary']).pack(fill="x", pady=2)
        ModernButton(rb, text="🌐 Translate", command=self.perform_translation, 
                    bg_color=self.COLORS['warning']).pack(fill="x", pady=2)
        ModernButton(rb, text="🧹 Clear", command=self.clear_text, 
                    bg_color=self.COLORS['danger']).pack(fill="x", pady=2)
        
        out = tk.LabelFrame(right.inner, text="Output", bg=self.COLORS['background'])
    ...
    def make_card(self, parent, title, color, size=12):
        f = tk.Frame(parent, bg=self.COLORS['card'], bd=1, 
                    highlightbackground="#1E293B", highlightthickness=1)
        f.pack(fill="x", padx=10, pady=5)
        tk.Label(f, text=title, font=("Segoe UI", 9), bg=self.COLORS['card'], 
                fg=self.COLORS['muted']).pack(anchor="w", padx=5)
        l = tk.Label(f, text="...", font=("Segoe UI", size, "bold"), 
                    bg=self.COLORS['card'], fg=color, wraplength=280, justify="left")
    ...
    def start_listen(self):
        ...
        self.stop_btn.pack(pady=10, ipadx=20)
        self.update_speak_status("Listening...", 'info')
        
        # Start waveform animation thread
        threading.Thread(target=self.animate_siri_wave, daemon=True).start()
    ...
    def stop_listen(self):
        ...
        self.stop_btn.pack_forget()
        self.speak_btn.pack(pady=10, ipadx=20)
    ...
    def listen_thread(self):
        try:
            ...
            self.message_queue.put((
                'status',
                f"Could not recognize {self.speak_lang.get()} speech",
                'danger'
            ))
    ...
    def translate_wrapper(self, text, src_code=None, dest_code='en'):
        ...
        try:
            translated_text = GoogleTranslator(
                source=src_code if src_code else "auto",
                target=dest_code
            ).translate(text)
    ...
    def groq_language_ratio(self, text, selected_lang_code):
        """
        Returns True if selected language >= 50% dominant.
        ...
        try:
                ...
                messages=[
                    {
                        "role": "system",
                        "content": (
                            "Detect language distribution of the sentence.\n"
            ...
            if start == -1 or end == -1:
                print("Invalid JSON format from Groq")
                return False
            ...
            ratio = float(lang_dict.get(selected_lang_code, 0))

            print("Language ratios:", lang_dict)

            return ratio >= 0.5

        except Exception as e:
            print("Groq language ratio error:", e)
            return False
        
    def _language_check_thread(self, text, src_code):
        allowed = self.groq_language_ratio(text, src_code)

        if not allowed:
            self.after(0, lambda: self.update_speak_status(
                "Please speak in selected language (>=50%)",
                "danger"
            ))
            return

        # If allowed → continue pipeline
        self.after(0, lambda: self._continue_speech_pipeline(text, src_code))
    ...
    def process_speech_text(self, text):
        ...
        threading.Thread(
            target=self._language_check_thread,
            args=(text, src_code),
            daemon=True
        ).start()
        return

    def _continue_speech_pipeline(self, text, src_code):
        ...
        if not english_text:
            self.update_speak_status("Translation failed", 'danger')
            return
    ...
    def cam_loop(self):
        ...
        while self.running:
            ...
            if fres.multi_face_landmarks and hres.multi_hand_landmarks:
                ...
                feat = np.concatenate([l.flatten(), r.flatten(), 
                                      [np.linalg.norm(lc-fc)], [np.linalg.norm(rc-fc)], 
                                      np.zeros(8)])[None,:]
            ...
            self.vid.after(0, lambda i=img: self.vid.config(image=i))
            self.vid.image = img
        cap.release()
        
    def _ai_refine_thread(self, raw_txt):
        """Threaded function to refine text using Groq and update the UI."""
        enhanced = self.groq_refine_sentence(raw_txt)

        # Update AI label on main thread
        self.after(0, lambda: self.ai_lbl.config(text=enhanced))

        # Run translation AFTER AI refinement
        self.after(0, lambda: self._translate_after_ai(enhanced))

    def _translate_after_ai(self, text):
        """Translates the refined AI text into the target language."""
        dest_code = LANGUAGES[self.trans_lang.get()]['code']

        try:
            res = self.translate_wrapper(text, src_code='en', dest_code=dest_code)

            if res:
                self.translated_sentence = res.text
                self.phonetic_sentence = res.pronunciation or res.text
    ...
    def perform_translation(self):
        """Main entry point for converting sign sequence to text/translation."""
        ...
        if self.use_ai_sentence.get():
            # Run AI refinement in a background thread to prevent UI freezing
            threading.Thread(
                target=self._ai_refine_thread,
                args=(raw_txt,),
                daemon=True
            ).start()
        else:
            # Skip AI refinement and translate raw text directly
            ...
            try:
                ...
                print("Translation Logic Error:", e)
    ...
    def clear_text(self):
        ...
        self.phonetic_sentence = ""
        self.translated_sentence = ""
        self.w_lbl.config(text="...")
        self.s_lbl.config(text="...")
